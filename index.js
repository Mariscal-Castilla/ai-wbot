	...
	client.ev.on("messages.upsert", async m => {
		...
		switch (command) {
			case "ping": {
				...
				await client.sendMessage(from, { video: { url: "./gif.mp4" }, gifPlayback: true, caption: `*[ ${(Math.floor(sec / (24 * 60 * 60))).toString().padStart(2, "0")}:${(Math.floor((sec % (24 * 60 * 60)) / (60 * 60))).toString().padStart(2, "0")}:${(Math.floor((sec % (60 * 60)) / 60)).toString().padStart(2, "0")}:${(sec % 60).toString().padStart(2, "0")} ]*` })
				break
			}
			case "restart": {
				if (!isOwner) return
